/* ============================================================
   MIMIC-IV Acute Pancreatitis (AP) Cohort Extraction
   FINAL OPTIMIZED VERSION - 2025 ML Feature Enhanced
   
   Features:
   1. Validated ItemIDs for high-missing variables (NLR, D-Dimer, etc.)
   2. Dynamic slopes for Glucose and SpO2
   3. Persistent Organ Failure (POF) definition (48h rule)
   4. Ranson/APACHE relevant biomarkers (LDH, Glucose, AST/ALT/LDH)
============================================================ */
...
-- 4. Batch lab slopes (Optimized with Verified ItemIDs for NLR, D-Dimer, Glucose)
DROP TABLE IF EXISTS temp_lab_slopes;
CREATE TEMP TABLE temp_lab_slopes AS
WITH lab_agg AS (
    SELECT 
        le.hadm_id,
        -- NLR Components: Combining top ItemIDs for max coverage
        ...
        -- Coagulation & Necrosis: Combining D-Dimer ItemIDs
        ...
        -- Metabolism & Nutrition (Glucose from Chemistry & Blood Gas)
    ...
    WHERE le.charttime BETWEEN c.intime AND c.intime + INTERVAL '24 hours'
      ...
      -- Filter for specific verified item IDs only
      AND itemid IN (51256, 52075, 51244, 51133, 50915, 51196, 52551, 51214, 50954, 50931, 50809, 52569, 51277, 52172, 51000, 50907, 50889, 50971, 50956)
    GROUP BY le.hadm_id
),
bg_agg AS (
    SELECT 
