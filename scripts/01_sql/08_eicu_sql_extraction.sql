...
WITH lab_filt AS (
    SELECT 
        ...
        CASE 
            -- 1. BUN 单位 mg/dL (已按要求取消转换补丁，保持原始 mg/dL)
            ...
            -- 3. Hemoglobin & Hematocrit (MIMIC 模型需要 Hct)
...
SELECT 
    ...
    -- 肝功 (补全 bilirubin_max 用于 tbar 计算)
...
WITH tier_calc AS (
    SELECT
        c.patientunitstayid, c.uniquepid, c.age AS admission_age, c.gender, c.bmi, c.icu_los_hours,
        ...
        l.hematocrit_max,
        l.hematocrit_min,
        l.wbc_max, l.wbc_min, l.ptt_min, l.albumin_min, l.albumin_max,
        l.ast_max, l.alt_max, l.alp_max,
        l.bilirubin_total_min,
        l.hemoglobin_min,
        l.glucose_lab_max,
        
        -- 计算 tbar (已补逗号)
        CASE WHEN l.albumin_min > 0 THEN l.bilirubin_total_max / l.albumin_min ELSE NULL END AS tbar,
